...
class READ_Bought_History():
    ...
    def __init__(self):
        ...
        self.item_card = MCard()
        self.item_card.setbase(2**24,6)  # 参数设置
        self.item_card.set_card()  #初始化hash桶        
        self.item_array = np.zeros((1000000,2),int)
        self.item_user_list = []
        # 商品分类信息
        self.item_class = np.zeros([0] * 20)  # 记录下标对应商品的类别
        self.class_dict = {}
    ...
    def item_hot(self):
        ...
        for i_record in xrange(0, self.record_num + 1):
            ...
            item_index = item_card.get(item_id, -1)
            if item_index == -1:
                self.item_num += 1  # 商品数+1
                item_index = self.item_num  # 商品分配的存储位置
                item_card[item_id] = item_index  # 记录商品存储的未知
        ...
        for x in xrange(0, self.item_num + 1):
            self.item_card.update_card(str(self.item_array[x, 0]))
    ...
    def read_class_id(self):
        ...
        for line_i in read_stream:
            ...
            item_index = self.item_card.read_card(aa[0]) - 1
            # 购买记录中存在该商品
            if item_index != -1:
    ...
    def class_item_hot(self):
        ...
        for i_record in xrange(0, self.record_num + 1):
            ...
            if temp[1] == 0:
                temp_array = np.array([-1] * self.num_k)
                index_temp = 0
            item_index = self.item_card.read_card(str(temp[0])) - 1  # 最后一行为残余项求和
            class_index = self.class_dict[self.item_class[item_index]]  # 商品类别列编号
            ...
            for x in xrange(0, self.num_k):
                ...
                if class_id0 == -1:
                    break
                self.matrix[item_index,class_id0] += 1
